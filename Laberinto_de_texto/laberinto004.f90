...
case ("guardar")
    ...
    print*, "Progreso guardado en ", trim(archivo), ".txt"
...
subroutine guardar_juego(nombre, nom1, nom2, nom3, sal3, sal1, current_pos, cont, z)
    ...
    archivo = "saves/" // trim(nombre) // ".txt"
    open(unit=23, file=archivo, status='replace', iostat=ios)
    if (ios /= 0) then
        print*, "Error al guardar el archivo ", archivo
        return
    end if

    ! Guardar los 4 vectores
    do i = 1, n
        write(23,*) nom1(i), nom2(i), nom3(i), sal3(i), sal1(i), current_pos, cont, z
    end do
    close(23)
...
subroutine cargar_juego(nombre, nom1, nom2, nom3, sal3, sal1, current_pos, cont, z)
    ...
    ! Aquí podrías luego pedir al usuario que elija un archivo
    print*, "Introduce el nombre del archivo para cargar (sin extension):"
    print*
    read*, archivo
    archivo = "saves/" // trim(archivo) // ".txt"
    ndatos=0


    open(unit=25, file=archivo, status='old', iostat=ios)
    if (ios /= 0) then
    ...
    open (25, file=archivo)   
    do
        read(25,*, iostat=ios) aux
        if (ios /= 0) exit
       ndatos=ndatos+1
           end do 
close (25)
    ...
    open(unit=25, file=archivo, status='old', iostat=ios)
    do i = 1, ndatos
        read(25,*) nom1(i), nom2(i), nom3(i), sal3(i), sal1(i), current_pos, cont, z
    end do
    close(25)
    print*
